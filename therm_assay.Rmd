...
therm = ib.variables.sum %>% 
  left_join(ibkey %>% select(assay_id, mix, rep), by = 'assay_id')
...
ib.cut2 = ib.cut2 %>% 
  left_join(ibkey %>% select(assay_id, mix))
  #refactor
  ib.cut2$mix[ib.cut2$mix %in% c("ii", "ti")] = "inbred"
  ib.cut2$mix[ib.cut2$mix %in% c("tx", "ix")] = "hybrid"
  ib.cut2$mix[ib.cut2$mix %in% c("ti:ii")] = "mixed"
  ib.cut2$mix[ib.cut2$mix =="control"] = NA
  ib.cut2$mix = factor(ib.cut2$mix, levels = c("inbred", "hybrid", "mixed"))

group.colors = c("#648FFF", "#FFB000", "#DC267F")
  
therm.temps = 
ggplot(data = ib.cut2) +
  geom_line(aes(x = datetime, y = temp, color = mix, group = assay_id,
...
therm$cat = NA
  therm$cat[therm$mix == "ii" | therm$mix == "ti"] = "inbred"
  therm$cat[therm$mix == "ix" | therm$mix == "tx"] = "hybrid"
  therm$cat[therm$mix == "ti:ii"] = "mixed"
...
#estimated differences
  #thermal error
  c(test.ih.err$estimate, test.im.err$estimate, test.hm.err$estimate) %>%
      round(digits = 4) %>% as.character()
  #thermal variance
  c(test.ih.var$estimate, test.im.var$estimate, test.hm.var$estimate) %>%
    round(digits = 4) %>% as.character()
```
...
p.err = ggplot(therm2, aes(x = cat, y = meanerr)) +
  ...
  labs(y = "Hourly Error (\u00B0C)",  x = NULL, shape = "replicate") +
    ...
    comparisons = list(c("inbred", "hybrid"), c("inbred", "mix"), c("hybrid", "mix")),
...
p.var = ggplot(therm2, aes(x = cat, y = meanvar)) +
  #bars for means
  geom_col(data = therm.allmeans, aes(y = mean_allmwv, fill = cat),
           position = "dodge", alpha = 0.5, width = 0.5,
           show.legend = F) + 
  #individual points
...
pbar.var = ggplot(therm2, aes(x = cat, y = meanvar)) +
  #bars for means
  geom_col(data = therm.means, aes(y = mean_repmwv, fill = rep),
           position = "dodge", alpha = 0.5, width = 0.5) + 
  #individual points
